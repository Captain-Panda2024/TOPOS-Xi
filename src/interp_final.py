class Interpreter:
        ...
        for node in ast:
            if isinstance(node, SpaceDef):
                for member in node.members:
                    if isinstance(member, MappingDef):
                        self.mappings[member.name] = member
    ...
    def generic_execute(self, node: ASTNode): pass
    ...
    def execute_MappingDef(self, node: MappingDef, args: List[Any] = None, depth: int = 0):
        if depth > self.max_depth: raise RuntimeError(f"Recursion depth exceeded")
        indent = "  " * (depth + 1)
